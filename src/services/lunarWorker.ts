 ...
 *
 * 使用 Reanimated 4.1 的 createWorkletRuntime + runOnRuntime
...
// 创建独立的 worklet runtime
const lunarRuntime = createWorkletRuntime({ name: "lunar" });
...
export const getLunarInfoBatchAsync = (
  ...
  return new Promise<Map<string, SerializableLunarInfo>>((resolve) => {
    ...
    const worklet = runOnRuntime(lunarRuntime, (y: number, m: number, ws: Day) => {
      "worklet";
...
/**
 * 异步计算整月的法定假日日期集合（在 worklet 线程执行）
 * @param year 年份
 * @param month 月份（0-indexed，0 = 一月）
...
export const getStatutoryHolidaySetForMonthAsync = (
  ...
  return new Promise<Set<string>>((resolve) => {
    // 在 worklet 内部计算并直接调用 runOnJS 返回结果
    const worklet = runOnRuntime(lunarRuntime, (y: number, m: number) => {
      "worklet";
